...
#let formulas = (
  text(size: 25pt, weight: "bold", "Unir Física"),
  ...
  $k = omega sqrt(epsilon mu/ 2) sqrt(sqrt(1+(sigma/(epsilon omega))^2)+1)$,
  $kappa = omega sqrt(epsilon mu/ 2) sqrt(sqrt(1+(sigma/(epsilon omega))^2)-1)$,
  $delta = 1 slash kappa$,
  $phi.alt_2 - phi.alt_1 = (2 pi d sin theta) slash lambda$,
  $x_K = K lambda D slash d$,
  $x_(K-1) - x_K = lambda D slash d$,
  text(size: 25pt, weight: "bold", "Óptica Física"),
)
